...
def Dice_loss(inputs, target, beta=1, smooth=1e-5):
    ...
    if target.dim() == 4:
        # 检查是否是 (N, H, W, C) 格式 (旧版 DataLoader 产生的格式)
        if target.size(3) == c and target.size(1) != c:
            # 把它转回 (N, C, H, W)
        ...
        # 尺寸对齐
        nt, ct, ht, wt = target_one_hot.size()
        if h != ht and w != wt:
            inputs = F.interpolate(inputs, size=(ht, wt), mode="bilinear", align_corners=True)
        
        # 通道对齐 (106 vs 105 问题)
        if ct > c:
            target_one_hot = target_one_hot[:, :c, :, :]

    # 2. 如果 Target 是 3D (新版 DataLoader, 推荐!)
    elif target.dim() == 3:
    ...
    inputs_flat = inputs_softmax.contiguous().view(n, c, -1)
    target_flat = target_one_hot.contiguous().view(n, c, -1)
