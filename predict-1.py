...
def detect_image(file_path, model, num_classes, exp_folder, mix_type=True):
    ...
    # 生成颜色映射
    random.seed(42) # 固定随机种子，保证每次跑颜色一样
    ...
    for i in range(num_classes - 1):
        # 使用黄金分割比来生成差异最大的色相，避免相邻ID颜色相近
        hue = (i * 0.618033988749895) % 1.0 
        sat = 0.7 + random.random() * 0.3  # 饱和度 0.7-1.0
        val = 0.7 + random.random() * 0.3  # 亮度 0.7-1.0
    ...
    with torch.no_grad():
        ...
        pr = pr.argmax(axis=-1)
        unique_classes = np.unique(pr)
        print(f"Image {os.path.basename(file_path)} detected classes: {unique_classes}")
    ...
    print(f"Mask s1aved at: {save_path}")
...
def parse_args():
    ...
    # 添加数据路径参数
    parser.add_argument("--data_path", default="/home/u241003661121/U-Net/FoodSeg103/VOC2012/JPEGImages/00000208.jpg",
                        help="data root")
    # 添加模型权重路径参数
    parser.add_argument("--weights", default="run/train/exp100/weights/best_model_104.pth")
    # 添加类别数量参数，默认为 3
