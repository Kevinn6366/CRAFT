...
def train(args):
    ...
    optimizer, lr_scheduler_func = get_optimizer_and_lr(model, batch_size, total_epochs, args.momentum,
...
def train_one_epoch(model, optimizer, data_loader, device, dice_loss, focal_loss, gpu_used, num_classes, scaler, epoch,
                    total_epochs, writer):
    model.train()
    total_loss = 0.0
    total_loss_main = 0.0
    ...
    for iteration, batch in enumerate(pbar):
        ...
        if scaler is not None:
            with torch.cuda.amp.autocast():
                ...
                target_small = F.interpolate(pngs.unsqueeze(1).float(), scale_factor=0.25, mode='nearest').squeeze(
                    1).long()
                l_aux = 0
                if focal_loss:
...
def parse_args():
    ...
    parser.add_argument("--momentum", type=float, help="Optimizer momentum (beta1)")
    parser.add_argument("--weight-decay", type=float, dest='weight_decay', help="Weight decay")
    parser.add_argument("--amp", type=bool, help="Use torch.cuda.amp for mixed precision")
    parser.add_argument("--start-epoch", type=int, help="Start epoch for resuming training")
